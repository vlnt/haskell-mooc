...
leftpad str i = 
    if length str >= i
        then str
    else let num = i - length str
    in
        concat(replicate num " ") ++ str
...
countdown n = helper n ""
helper n res = 
        if n == 0
            then "Ready! " ++ res ++ " Liftoff!"
        else helper (n - 1) (res ++ (show n ++ "... "))
...
smallestDivisor n = helper_sD n 2
helper_sD n k =
        if mod n k == 0
            then k
        else helper_sD n (k + 1) 
